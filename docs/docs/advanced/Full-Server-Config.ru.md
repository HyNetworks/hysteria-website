...
8. Отключить QUIC stateless reset. Stateless reset позволяет клиенту, удерживающему соединение, о котором сервер уже не знает (например, после перезапуска), переподключиться сразу, а не ждать истечения своего таймаута простоя. При отключении сервер в такой ситуации просто молчит.
...
2. URL проксируемого сайта. Также может быть Unix-сокетом, записанным как `unix:///path/to.sock` или как абсолютный путь вида `/path/to.sock`.
